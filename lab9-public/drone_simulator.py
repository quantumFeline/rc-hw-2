...
class DroneSimulator:
    def __init__(self, model, data, viewer, steps=1, view=True, wind_change_prob = 0, rendering_freq = 1):
        self.model = model
        self.data = data
        self.viewer = viewer
        self.steps = steps
    ...
    def sim_step(self, thrust, roll_thrust, pitch_thrust, yaw_thrust, steps=1, view=True):
        if np.random.uniform() < self.wind_change_prob:
            self.wind1 = np.random.uniform(-0.5, 0.5)
            self.wind2 = np.random.uniform(-0.5, 0.5)
            self.wind3 = np.random.uniform(-0.5, 0.5)
            self.wind4 = np.random.uniform(-0.5, 0.5)
